...
* init	создает ЛР
* remote	работает с УР
* add	добавляет файлы в индекс (под версионный контроль)
* status	отображает состояние файлов в ЛР
* commit	фиксирует текущее состояние файлов
* push	отправляет коммиты в УР
* log	отображает список коммитов
* clone	скачивает УР на локальный компьютер
* pull	подтягивает разницу коммитов между УР и ЛР